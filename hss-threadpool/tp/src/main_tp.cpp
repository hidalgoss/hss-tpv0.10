...
// En esta versión de la implementación no podemos utilizar f(x)'s con valroes de retorno 
// distintas a void ya que la queue del threadpool es de std::packaged_task<void()>.
// float task1(){
...
int main(){
  ...
  //hss::ThreadPool<std::function<void ()> , void> hssThreadPool;
  ...
  try
  {
    ...
    if (std::is_same<decltype(task2), void* >::value)
      std::cout << "type task2 == void*" << std::endl;
    if (std::is_same<decltype(task2), void() >::value)
      std::cout << "type task2 == void()" << std::endl;
